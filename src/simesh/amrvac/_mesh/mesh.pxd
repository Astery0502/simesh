...
cdef class AMRMesh:
    ...
    cpdef void first_derivative_fields(self, double[:,:,:,:,:] output, uint32_t[:] term_output_positions,
        uint32_t[:] term_field_positions, uint32_t[:] term_axes, double[:] term_coefficients)
    ...
    cdef void getbc(self, int workers=*, object executor=*)
    ...
    cdef void interpolation_linear(self, uint32_t ileaf, int ixFimin[3], int ixFimax[3],
        double dxFi[3], double xFimin[3], double dxCo[3], double invdxCo[3], double xComin[3]) noexcept nogil
