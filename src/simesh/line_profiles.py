...
def sample_line_profiles(fields, lines, components=None, *, point_batch=4096,
    ...
    with worker_context(workers) as executor:
        for first in range(0, count, point_batch):
            ...
            _sample(fields, points, workers, executor, field_positions,
...
def iter_line_profiles(fields, line_batches, components=None, *, memory_limit=None, **controls):
    ...
    for lines in line_batches:
        result = sample_line_profiles(fields, lines, selected,
            memory_limit=remaining(memory_limit, previous), **controls)
        previous = result.nbytes
        yield result
        del result

__all__ += ["iter_line_profiles"]
