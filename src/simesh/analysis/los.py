...
def integrate_los(fields,plane,direction,*,component=0,near=0.,far=np.inf,
    ...
    direction = np.array(direction,dtype=float,order="C",copy=True)
    if direction.shape!=(3,) or not np.isfinite(direction).all() or not np.any(direction):
        raise ValueError("LOS direction must be a finite nonzero triplet")
    ...
    required = footprint+nx*ny*96+min(tx,nx)*min(ty,ny)*384
    ...
    return LOSResult(plane,direction,*arrays,definitions[component].units+" * coordinate-length",quadrature)
