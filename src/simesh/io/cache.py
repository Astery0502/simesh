...
class InteriorValueCache:
    ...
    def read_native(self, ids, fields, target):
        zero = np.zeros(3,dtype=np.int64)
        upper = np.asarray(self.backing_reader.shape[2:],dtype=np.int64)
        for first in range(0,len(ids),self.capacity):
            last = min(first+self.capacity,len(ids))
            read_blocks_into(self.reader,zero,upper,ids[first:last],fields,
                             np.moveaxis(target[first:last],-1,1),zero)
...
def cache_source(source, *, capacity, fields=None, memory_limit=None):
    ...
    if fields is not None:
        ...
        source = select_source(source, fields)
    if type(capacity) is not int or capacity<1:
