...
def read_full_interiors(reader, field_ids, *, chunk_bytes=READ_BATCH_BYTES, output=None):
    """Read contiguous spans into field-major interiors, optionally a caller view.
    ...
    failure returns no result but can leave caller output partially written.
    The source identity is checked before and after.
    """
    state = reader.state
    ...
    elif (output.shape != (count,len(fields),*state.shape[2:]) or
          output.dtype != np.float64 or not output.flags.writeable):
        raise ValueError("output must match writable field-major interiors")
