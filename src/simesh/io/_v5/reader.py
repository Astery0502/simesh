...
from simesh.io._v5.index import AMRVACV5ForestBinding, AMRVACV5Index
from simesh._amr.blockio import BlockReader, make_block_reader
...
def _require_destination(value: np.ndarray, *, native_output=False) -> np.ndarray:
    if not isinstance(value, np.ndarray):
    ...
    if not native_output and not value.flags.c_contiguous:
...
def _read_amrvac_v5_blocks_into(
    ...
    *, native_output=False,
    ...
    destination = _require_destination(destination, native_output=native_output)
