...
# ============================================================================
# PARALLEL READING IMPLEMENTATIONS
# ============================================================================

def read_multiple_blocks_mmap_parallel(filename, block_offsets, block_shape, 
                                       field_indices=None, ndim=3, max_workers=8):
    """
    Read multiple blocks in parallel using a single mmap and threading.
    This is the MOST EFFICIENT approach - single file open, parallel memory access.
    
    Parameters
    ----------
    filename : str
        Path to the datfile
    block_offsets : list of int
        Byte offsets for each block
    block_shape : array-like
        Shape of each block (nx, ny, nz)
    field_indices : list of int, optional
        Which field indices to read (None = all)
    ndim : int
    ...
    Returns
    -------
    list of arrays : One array per block, shape depends on field_indices
    
    Notes
    -----
    - Uses single mmap, shared across threads (thread-safe for reads)
    ...
    # Read header BEFORE opening mmap
    ...
    with open(filename, 'rb') as f:
        # Single mmap creation - this is shared across all threads
        with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
            
            def read_single_block_from_mmap(offset):
                """Worker function - reads one block from the shared mmap"""
                count = np.prod(block_shape)
                byte_size_field = count * SIZE_DOUBLE
                ghost_offset = 2 * ndim * SIZE_INT
                
                # Read ghost cells
                ghostcells = np.frombuffer(mm, dtype='=i4', count=2*ndim, offset=offset)
                ghostcells = ghostcells.reshape(2, ndim).copy()  # Copy to avoid mmap dependency
                ...
                for field_idx in field_indices:
                    ...
                    arr = arr.reshape(block_shape[::-1]).T
                    
                    # Expand to 3D if needed
                    while len(arr.shape) < 3:
                        arr = arr[..., np.newaxis]
                    
                    block_fields.append(arr.copy())  # Copy to avoid mmap dependency
                
                return ghostcells, np.array(block_fields)
            
            # Parallel execution with ThreadPoolExecutor
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                results = list(executor.map(read_single_block_from_mmap, block_offsets))
            
            return results


def write_multiple_blocks_mmap_parallel(filename, block_data_list, block_offsets,
                                        ndim=3, max_workers=8):
    """
    Write multiple blocks in parallel using mmap.
    
    Parameters
    ----------
    filename : str
    block_data_list : list of arrays
        Each element is a block's data array
    block_offsets : list of int
        Byte offsets for each block
    ndim : int
    max_workers : int
    ...
    with open(filename, 'r+b') as f:
        with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_WRITE) as mm:
            ...
            def write_single_block(args):
                """Worker function"""
                block_data, offset = args
                
                # Write ghost cells (zeros for simplicity)
                ghostcells = np.zeros(2 * ndim, dtype=np.int32)
                mm.seek(offset)
                mm.write(ghostcells.tobytes())
                
                # Write block data (transpose for Fortran order)
                if len(block_data.shape) == 4:  # (nx, ny, nz, nw)
                    block_data_transposed = np.transpose(block_data, (3, 2, 1, 0))
                else:
                    block_data_transposed = block_data.T
                
                mm.write(block_data_transposed.flatten().astype('=f8').tobytes())
            
            # Parallel writing
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                list(executor.map(write_single_block, zip(block_data_list, block_offsets)))


def benchmark_file_open_cost(filename, n_opens=100):
    """
    Benchmark the cost of opening a file multiple times.
    
    Parameters
    ----------
    filename : str
    n_opens : int
        Number of times to open the file
...
# ============================================================================
# ADVANCED: mmap + asyncio + multiprocessing
# ============================================================================

async def read_blocks_async_mmap(filename, block_offsets, block_shape, 
                                  field_indices=None, ndim=3, max_concurrent=10,
                                  process_func=None):
    """
    Read multiple blocks asynchronously using mmap.
    ...
    process_func : callable, optional
        Function to process each block after reading: func(ghostcells, fields) -> result
        
    Returns
    -------
    list : Results from processing each block
    ...
    >>> def process_block(ghostcells, fields):
    ...     # Do CPU work on the block
    ...     density = fields[0]
    ...     return density.mean(), density.std()
    >>> 
    >>> # Run async reading
    >>> results = asyncio.run(read_blocks_async_mmap(
    ...     "file.dat", offsets, (16,16,16), 
    ...
    with open(filename, 'rb') as f:
        # Single mmap, shared across all async tasks
        with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
            
            async def read_and_process_block(offset):
                """Async worker - reads and optionally processes a block"""
                # Read from mmap (fast, memory-backed)
                count = np.prod(block_shape)
                byte_size_field = count * SIZE_DOUBLE
                ghost_offset = 2 * ndim * SIZE_INT
                ...
                fields = np.array(block_fields)
                
                # Optionally process the block
                if process_func is not None:
                    # Allow I/O to happen while processing
                    await asyncio.sleep(0)  # Yield control
                    result = process_func(ghostcells, fields)
                    return result
                else:
                    return ghostcells, fields
            
            # Create semaphore to limit concurrent operations
            semaphore = asyncio.Semaphore(max_concurrent)
            
            async def bounded_read(offset):
                async with semaphore:
                    return await read_and_process_block(offset)
            
            # Execute all reads concurrently
            tasks = [bounded_read(offset) for offset in block_offsets]
            results = await asyncio.gather(*tasks)
            
            return results


def read_blocks_multiprocessing_mmap(filename, block_offsets, block_shape,
                                      field_indices=None, ndim=3, n_processes=None,
                                      process_func=None, chunk_size=None):
    """
    Read and process multiple blocks using multiprocessing with mmap.
    Best for: CPU-intensive processing after reading.
    
    Each process opens its own mmap (cheap ~1ms), then processes assigned blocks.
    ...
    process_func : callable, optional
        Function to process each block: func(ghostcells, fields) -> result
    chunk_size : int, optional
        Blocks per process per iteration (default: auto)
    ...
    Example
    -------
    >>> def compute_statistics(ghostcells, fields):
    ...     density = fields[0]
    ...     # Heavy computation
    ...     return {
    ...         'mean': density.mean(),
    ...
    ...     }
    >>> 
    >>> results = read_blocks_multiprocessing_mmap(
    ...     "file.dat", offsets, (16,16,16),
    ...     process_func=compute_statistics,
    ...     n_processes=8
    ... )
    """
    ...
    def worker_read_and_process(offset):
        """
        Worker function - each process opens mmap and processes blocks.
        Opening mmap per process is cheap (~1ms) and allows true parallelism.
        """
        with open(filename, 'rb') as f:
            with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
                count = np.prod(block_shape)
                ...
                if process_func is not None:
                    return process_func(ghostcells, fields)
                else:
                    return ghostcells, fields
    ...
    with Pool(processes=n_processes) as pool:
        if chunk_size is None:
            ...
            chunk_size = max(1, len(block_offsets) // (n_processes * 4))
        
        results = pool.map(worker_read_and_process, block_offsets, chunksize=chunk_size)
    
    return results


def read_blocks_hybrid(filename, block_offsets, block_shape,
                        field_indices=None, ndim=3,
                        n_processes=None, blocks_per_process=None,
                        process_func=None):
    """
    HYBRID: Multiprocessing + mmap for optimal performance.
    
    Strategy:
    - Divide blocks among processes
    - Each process opens ONE mmap
    - Each process reads multiple assigned blocks from its mmap
    - Optionally processes blocks with CPU
    
    This combines:
    - Single mmap per process (no reopening overhead)
    - True parallelism via multiprocessing
    - Efficient random access via mmap
    
    Best for: Large datasets with both I/O and CPU work.
    ...
    n_processes : int, optional
        Number of processes (default: CPU count)
    blocks_per_process : int, optional
        How many blocks each process handles (default: auto-balance)
    process_func : callable, optional
    ...
    Example
    -------
    >>> results = read_blocks_hybrid(
    ...     "file.dat", 
    ...     offsets, 
    ...     (16,16,16),
    ...     n_processes=8,
    ...     process_func=lambda g, f: f[0].mean()  # density mean
    ... )
    """
    import multiprocessing as mp
    from multiprocessing import Pool
    ...
    # Get header (read once, shared info)
    with open(filename, 'rb') as f:
        header = get_header(f)
        nw = header['nw']
    ...
    if blocks_per_process is None:
        blocks_per_process = (n_blocks + n_processes - 1) // n_processes
    
    # Create chunks for each process
    offset_chunks = []
    for i in range(0, n_blocks, blocks_per_process):
        chunk = block_offsets[i:i + blocks_per_process]
        offset_chunks.append((i, chunk))  # (start_index, offsets)
    
    def worker_process_chunk(args):
        """
        Each process:
        1. Opens mmap ONCE
        2. Reads multiple blocks from the same mmap
        3. Processes blocks
        """
        start_idx, offsets = args
        results = []
        
        # Open mmap once per process (file handle doesn't need header read here)
        with open(filename, 'rb') as f:
            with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
    ...
    # Execute in parallel
    with Pool(processes=n_processes) as pool:
        chunk_results = pool.map(worker_process_chunk, offset_chunks)
    
    # Reconstruct results in original order
    all_results = [None] * n_blocks
    for start_idx, results in chunk_results:
...
def read_blocks_smart(filename, block_offsets, block_shape,
                       field_indices=None, ndim=3,
                       method='auto', n_workers=None,
                       process_func=None, **kwargs):
    """
    SMART DISPATCHER: Automatically choose the best method based on your needs.
    ...
    ndim : int
    method : str
        - 'auto': automatically choose based on conditions
        - 'threading': mmap + threading (good for I/O-bound)
        - 'asyncio': mmap + asyncio (good for many concurrent operations)
        - 'multiprocessing': true parallelism (good for CPU-bound)
        - 'hybrid': multiprocessing + mmap (BEST for large datasets)
    ...
    **kwargs : additional arguments for specific methods
    ...
    - Small dataset (< 50 blocks): threading
    - Many blocks + no processing: asyncio
    - Heavy processing: hybrid or multiprocessing
    ...
    if method == 'auto':
        ...
        if n_blocks < 50 and process_func is None:
            method = 'threading'
        elif process_func is not None:
            method = 'hybrid'  # Best for processing
        else:
            method = 'asyncio'  # Good for I/O
    ...
    if method == 'threading':
        return read_multiple_blocks_mmap_parallel(
            filename, block_offsets, block_shape,
            field_indices, ndim, max_workers=n_workers
        )
    
    elif method == 'asyncio':
        import asyncio
        max_concurrent = kwargs.get('max_concurrent', 10)
        return asyncio.run(read_blocks_async_mmap(
            filename, block_offsets, block_shape,
            field_indices, ndim, max_concurrent,
            process_func
        ))
    
    elif method == 'multiprocessing':
        chunk_size = kwargs.get('chunk_size', None)
        return read_blocks_multiprocessing_mmap(
            filename, block_offsets, block_shape,
            field_indices, ndim, n_workers,
            process_func, chunk_size
        )
    
    elif method == 'hybrid':
        blocks_per_process = kwargs.get('blocks_per_process', None)
        return read_blocks_hybrid(
            filename, block_offsets, block_shape,
            field_indices, ndim, n_workers,
            blocks_per_process, process_func
        )
    
    else:
        raise ValueError(f"Unknown method: {method}. Choose from: auto, threading, asyncio, multiprocessing, hybrid")
