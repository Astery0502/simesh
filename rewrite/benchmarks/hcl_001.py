"""HCL-001 traversal, amplification, compatibility, and allocation probe."""
    ...
    while first < face_neighbor_ids.shape[0]:
        if compatibility:
            primary_count, selected_count = plan_level1_halo_chunk(
                first, face_neighbor_ids, selected_ids
            )
        else:
...
def measure_capacity(
    ...
    chunks, selected_total = traverse(face_neighbor_ids, selected_ids, False)
    if traverse(face_neighbor_ids, selected_ids, True) != (chunks, selected_total):
    ...
    plan_level1_halo_closed_prefix(0, face_neighbor_ids, selected_ids)
...
def small_reference_probe(repeats: int) -> dict:
    root_shape = np.asarray([6, 5, 4], dtype=np.int64)
    ...
    expected = plan_level1_halo_closed_prefix_reference(0, faces, capacity)
    actual = plan_level1_halo_closed_prefix(0, faces, selected)
    if actual != expected[1:] or selected[: actual[1]].tolist() != expected[0]:
        raise AssertionError("small HCL reference disagreement")
    return {
        "blocks": int(faces.shape[0]),
        "capacity": capacity,
        "production_seconds": median_seconds(
            lambda: plan_level1_halo_closed_prefix(0, faces, selected), repeats
        ),
        "reference_seconds": median_seconds(
            lambda: plan_level1_halo_closed_prefix_reference(0, faces, capacity),
            repeats,
        ),
    }
...
def main() -> None:
    ...
    if minimum_halo_closed_slots(faces) != minimum:
        raise AssertionError("canonical and compatibility minimum disagree")
    minimum_seconds = median_seconds(
        lambda: minimum_level1_halo_closed_slots(faces), args.repeats
    )
    compatibility_minimum_seconds = median_seconds(
        lambda: minimum_halo_closed_slots(faces), args.repeats
    )
    report = {
        "capability": "HCL-001",
