...
| --- | --- | --- | --- |
| Array/index/layout conventions | `amrvac/layouts.py`, canonical array APIs | Core 3D conventions, workspace accounting, dense and sparse selected-primary traversal, level-1 closures, and refined source-union planning complete; singleton-z pending | M2 singleton-z, then M6 public integration |
...
| Mesh geometry and coordinate bookkeeping | `utils/lib/amr/mesh.pyx` | Level-1/refined Cartesian 3D bounds, spacing, and exact refined point ownership complete | M1 native analysis integration, then M2/M3 |
| Physical, sibling, coarse/fine and periodic halos | `amrvac/boundary.py`, `mesh.pyx` | Physical/level-1 sibling boundaries plus selected Cartesian 3D refined support, SAME/FINER/COARSER application, PBC-aware slope completion, and bounded complete-halo reader/writer execution are complete for `B>=4`, side reach `<=B/2`; 2D/periodic and wider-reach variants pending | M1 refined complete, then M2 and M3 |
| Exact/uniform sampling and uniform-to-SFC placement | `mesh.pyx`, `amrvac_uniform.py` | Refined repeated sampling, exact hinted ownership, and a completed-owner sampling cache are complete | M1 native/streamline integration, then M2/M3/M5 |
| Pointwise, stencil, reduction and diagnostics | `derived_fields.py`, `mesh.pyx`, legacy diagnostic evidence | Native selected refined curl/reduction plus normalized RHS, RK4, termination, and cached field-line execution complete | M1 horizon review; M4 for breadth |
