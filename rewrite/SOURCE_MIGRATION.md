...
| --- | --- | --- | --- |
| Array/index/layout conventions | `amrvac/layouts.py`, canonical array APIs | M1 3D conventions complete; DIM-001 explicit active-dimension/singleton-z provenance is the active M2 foundation | M2 singleton-z, then M6 public integration |
...
| Pointwise, stencil, reduction and diagnostics | `derived_fields.py`, `mesh.pyx`, legacy diagnostic evidence | M1 selected curl/reduction plus normalized RHS, RK4, termination, and cached field-line execution complete | M2 representative 2D operator; M4 for breadth |
