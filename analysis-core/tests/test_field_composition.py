...
def test_composition_reserves_publication_scratch(monkeypatch, operation):
    ...
    with sm.source_from_arrays(mesh, np.ones((256, 1, 1, 1, 1)), ("value",)) as source:
        fields = sm.read_fields(source)
    # The values and final directory fit, but publication also needs temporary
    # leaf indices. Small blocks expose this independently of recipe scratch.
    ...
    with pytest.raises(MemoryError):
        if operation == "select":
            ...
            merge_fields((fields,), memory_limit=limit)
        else:
            derive_many(fields, {"copy": "code"}, forbidden, memory_limit=limit+16)
