...
def open_amrvac(path, *, units=None, memory_limit=None):
    ...
    try:
        ...
        def read_full(fields):
            from ._bulk import read_full_interiors
            return read_full_interiors(reader, fields)

        source = Source(mesh, definitions, reader, validate=validate, close=lambda: os.close(fd),
                        read_full=read_full, read_scratch_bytes=2*max(maximum_record, 16*1024**2),
