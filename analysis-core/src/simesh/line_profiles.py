...
from .fields import _field_index, require_fields
...
def _components(fields, components):
...
class LineProfiles:
    ...
    def branch(self, seed_id, direction, *, memory_limit=None):
        ...
        admit(self.nbytes + (stop-start)*9, memory_limit, "profile branch view")
    ...
    def line(self, seed_id, *, memory_limit=None):
        ...
        admit(self.nbytes + count*(80+9*len(self.definitions)), memory_limit,
              "joined line profile")
    ...
    def _view(self, row, index, distance, point_indices, directions):
        ...
        return LineProfile(int(self.seed_ids[row]), positions, distance, values,
            ...
            self.termination[row], self.definitions, self.length_units,
...
def sample_line_profiles(fields, lines, components=None, *, point_batch=4096,
    ...
    selected = _components(fields, components)
