...
class _Decoder:
    ...
    def node(self, node, expected=None):
        ...
        for name, dtype in _ARRAYS[cls].items():
            ...
            # Own all arrays after closing the archive, in native byte order.
            data[name] = np.array(array, dtype=dtype, order="C", copy=True)
...
def save_result(path, result, *, metadata=None, source=None, overwrite=False):
    ...
    manifest = {"format": _FORMAT, "schema_version": SCHEMA_VERSION,
                ...
                "provenance": {"source": None if source is None else _json_object(source, "source"),
