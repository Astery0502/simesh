...
from .fields import _Lease, publish, source_value_identity
...
def iter_prepared(source, fields=None, *, region=None, leaf_ids=None, scheme,
    ...
    for first in range(0, len(selection.leaf_ids), batch_size):
        ...
        product = publish(source.mesh, output, selected, definitions, 2, 2,
                          ...
                          value_identity=source_value_identity(source.identity,field_ids,exact.SCHEME))
        try:
            yield product
        finally:
            lease.active = False
...
def _sample_pool(pool,points,workers,executor):
    ...
    while pending:
        ...
        with pool.borrow(ids) as fields:
            samples,_,okay=_sample(fields,points[first:last],workers,executor)
            values[first:last]=samples
...
def global_curl(source,fields=('b1','b2','b3'),*,batch_size=128,support_capacity=128,output=None,memory_limit=None):
    ...
    """
    import time
    from .fields import publish,source_value_identity
    from .mesh import Selection
    ...
    reserve=output_bytes+batch_bytes+mesh.leaf_count*32
    ...
    for primary in iter_prepared(source,chosen,scheme='exact-phase',batch_size=batch_size,
        ...
        derived=curl(primary,memory_limit=memory_limit)
        target[primary.leaf_ids]=derived.values
    ...
    return publish(mesh,view,Selection(mesh,ids),definitions,1,1,
        ...
        derivation=('curl',source_value_identity(source.identity,chosen,exact.SCHEME),
