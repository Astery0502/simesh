...
class FillPlan:
    ...
    def prepare(self,source,fields=None,*,memory_limit=None):
        ...
        for ids,count,actions in self.chunks:
            ...
            source.read_into(ids,fields,raw[:n])
            ...
            stamp=time.perf_counter()
            values[self.order[offset:offset+count]]=np.moveaxis(payload[:count],1,-1)
        ...
        return publish(self.mesh,values,self.selection,tuple(source.fields[i] for i in fields),
                       ...
                       value_identity=source_value_identity(source.identity,fields,self.scheme))
