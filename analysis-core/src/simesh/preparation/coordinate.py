...
from ..fields import Fields, source_value_identity
...
def prepare(source, fields=None, *, region=None, leaf_ids=None, workers=1,
    ...
    raw_bytes = n*k*math.prod(map(int, block))*8
    ...
    # Raw interiors are released before coarse scratch is allocated. Source
    # backing remains counted separately when the caller retains array input.
    required = (source.nbytes+mesh.nbytes+geometry_bound+n*24+k*32 +
                max(raw_bytes+source.read_footprint(n,k), final_bytes+raw_bytes,
                    final_bytes+coarse_bound))
    admit(required, memory_limit, "whole-domain preparation")
    started = time.perf_counter()
    geometry = build_geometry(mesh)
    planned = time.perf_counter()
    ids = np.arange(n, dtype=np.int64)
    if source._read_full is not None:
        raw, read_stats = source._read_full(chosen)
    else:
        raw = np.empty((n, k, *mesh.block_shape), dtype=float)
        source.read_into(ids, chosen, raw)
        read_stats = {"read_value_bytes": raw.nbytes, "selected_load_count": n}
    if (not isinstance(raw, np.ndarray) or raw.dtype != np.float64 or
            raw.shape != (n, k, *mesh.block_shape) or not raw.flags.c_contiguous):
        raise ValueError("full reader must return matching native field-major interiors")
    source.validate()
    read = time.perf_counter()
    # Both providers start from positive zero padding. NumPy directly owns this
    # final backing; later publication does not repack or copy it.
    values = np.zeros((n, *(int(b)+4 for b in block), k), dtype=float)
    allocated = time.perf_counter()
    values[:, 2:2+block[0], 2:2+block[1], 2:2+block[2], :] = np.moveaxis(raw, 1, -1)
    del raw
    ...
    stats = {"provider": "coordinate-phase", "workers": workers, "backend": backend,
             "geometry_seconds": planned-started, "read_seconds": read-planned,
             "allocation_seconds": allocated-read, "interior_copy_seconds": loaded-allocated,
    ...
    return Fields(mesh, values, selected, ids, tuple(source.fields[i] for i in chosen),
                  ...
                  value_identity=source_value_identity(source.identity,chosen,SCHEME))
