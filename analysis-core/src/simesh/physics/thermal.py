...
from ..fields import FieldDefinition, Fields, require_fields, publish
...
def thermal_fields(density,temperature,*,density_unit_g_cm3,model=AIA171(),density_component=0,
    ...
    """Detach number-density and kelvin nodes using exactly two valid input halos.
    ...
    if tproduct:
        require_fields(temperature,halo=2)
        ...
        temperature=value
    block=density.mesh.block_shape
    shape=(len(density.leaf_ids),*(n+4 for n in block),2)
    ...
    dh=density.storage_halo
    density_box=tuple(slice(dh-2,dh+n+2) for n in block)
    if tproduct:
        temperature_values=temperature.values
        temperature_slots=temperature.slot_of_leaf[density.leaf_ids]
        th=temperature.storage_halo
        temperature_box=tuple(slice(th-2,th+n+2) for n in block)
    ...
    return publish(density.mesh,values,density.selection,definitions,2,2,
