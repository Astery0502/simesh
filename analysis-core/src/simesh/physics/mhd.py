...
energy definition, composition or normalization. See docs/mhd-thermodynamics.md.
"""
...
from .._validation import admit, array_bytes, indices
from ..diagnostics import MagneticUnits
from ..fields import FieldDefinition, _field_index, publish, require_fields
...
def _select(fields, components, count):
    components = ((components,) if isinstance(components, (str, int, np.integer))
                  else tuple(components))
    selected = indices([_field_index(fields.fields, c) if isinstance(c, str) else c
                        for c in components], len(fields.fields), "components")
    if len(selected) != count:
        raise ValueError(f"select exactly {count} components")
    if count > 1 and len({fields.fields[i].units for i in selected}) != 1:
        raise ValueError("vector components must have common unit labels")
    return tuple(map(int, selected))


def _recover(rho_raw, momentum_raw, energy_raw, magnetic_raw, model):
    ...
    with np.errstate(divide="ignore", invalid="ignore", over="ignore", under="ignore"):
        ...
        sound_speed = np.sqrt(model.gamma) * (np.sqrt(pressure) / np.sqrt(rho))
        alfven_speed = b_over_sqrt_mu / np.sqrt(rho)
        ...
        np.divide(speed, alfven_speed, out=alfven_mach, where=valid & ~zero_b)
        sonic_mach = speed / sound_speed
        diagnostics = (sound_speed, alfven_speed, beta, sonic_mach, alfven_mach)
        for diagnostic in diagnostics:
            bad = valid & ~np.isfinite(diagnostic)
            if diagnostic is beta or diagnostic is alfven_mach:
                bad &= ~zero_b
            status[bad] |= int(MHDStatus.UNREPRESENTABLE_DIAGNOSTIC)
            diagnostic[~np.isfinite(diagnostic)] = np.nan
    values = {"density": (rho,), "velocity": velocity, "speed": (speed,),
              ...
              "sonic_mach": (sonic_mach,), "alfven_mach": (alfven_mach,)}
...
def mhd_fields(conserved, *, model, magnetic=None, density="rho",
    ...
    Include status to locate flags; preparation_stats always counts flags over
    interiors and all evaluated nodes separately. These are node counts, not
    volumes. Status fields are categorical and must not be interpolated.
    ...
    required = (conserved.mesh.nbytes + input_bytes + 8*math.prod(shape) +
                512*math.prod(block_shape) + 8*conserved.mesh.leaf_count)
    ...
    for row, leaf in enumerate(conserved.leaf_ids):
        ...
        recovered, status = _recover(c_values[(*c_node, rho_id)],
            ...
            tuple(b_values[(*b_node, i)] for i in b_ids), model)
