...
cdef class AMRMesh:
    ...
    cpdef object padded_view(self):
        ...
        result = cnp.PyArray_SimpleNewFromData(5, shape, cnp.NPY_FLOAT64, self._data_ptr)
        # A real NumPy base reference survives np.asarray, slicing and subclass
        # stripping. An attribute on a temporary ndarray subclass does not.
        cnp.set_array_base(result, self)
        return result
    ...
    cdef void _init_block_gridindex(self):
        ...
        for i in range(3):
            ...
            # A block may span both physical sides of a transverse axis.
            # Match the corrected independent coordinate-phase v2 support.
    ...
    cdef void _local_phase_range(self, int phase, uint32_t first, uint32_t last) noexcept nogil:
        cdef uint32_t ileaf
        cdef uint32_t[:,:] neighbors = self.forest.neighbor_type
        if phase == 0:
    ...
    cdef void _dispatch_local_phase(self, int phase, int workers, object executor):
        cdef int worker
        cdef uint32_t first, last
        if workers == 1:
            with nogil:
                self._local_phase_range(phase, 0, self.nleafs)
            ...
            try:
                for worker in range(workers):
                    ...
                    last = self.nleafs * (worker + 1) // workers
                    if first < last:
                        futures.append(executor.submit(_run_analysis_phase, self, phase, first, last))
                for future in futures:
                    future.result()
            finally:
                # Join even on submission/worker failure before retiring scratch.
                wait(futures)

    cdef void getbc(self, int workers=1, object executor=None):
        cdef uint32_t ileaf, i, j, k
        ...
        self._dispatch_local_phase(0, workers, executor)

        if needs_coarse:
            self._dispatch_local_phase(1, workers, executor)
        ...
        if needs_coarse:
            ...
            self._dispatch_local_phase(2, workers, executor)

        self._dispatch_local_phase(3, workers, executor)

    cdef void _coarsen_leaf(self, uint32_t ileaf, uint32_t[:,:] neighbor_type) noexcept nogil:
        cdef uint32_t i, j, k
        cdef uint32_t k_stop = 1 if self.ndim == 2 else 3
        cdef bint isboundary
        for i in range(3 ** self.ndim):
    ...
    cdef void fill_boundary_after_gc(self, uint32_t ileaf, uint32_t[:,:] neighbor_type) noexcept nogil:
        cdef uint32_t idim, i
        cdef int kmin[3]
        cdef int kmax[3]
        cdef int ixBmin[3]
    ...
    cdef void gc_prolong(self, uint32_t ileaf, uint32_t[:,:] neighbor_type) noexcept nogil:
...
def _run_analysis_phase(AMRMesh owner, int phase, uint32_t first, uint32_t last):
    """Private worker entry; the unpublished factory owner controls lifetime."""
    if phase < 0 or phase > 3 or first > last or last > owner.nleafs:
        raise ValueError("invalid local ghost range")
    with nogil:
        owner._local_phase_range(phase, first, last)


def _prepare_analysis_mesh(uint32_t[:] block, uint32_t[:] domain,
                           double[:] lower, double[:] upper, uint32_t nfields,
                           AMRForest forest, double[:, :, :, :, ::1] interiors, int workers=1, executor=None):
    """Build a private 3D two-halo owner and retire unborrowed exchange scratch.

    The validated analysis adapter owns admission and geometry checks. No view
    escapes this factory before exchange succeeds; mutable AMRMesh instances
    keep their existing load/reset and exported coarse-view lifetime.
    """
    cdef AMRMesh owner
    if workers < 1 or workers > 4:
        raise ValueError("preparation workers must be between one and four")
    if workers > 1 and executor is None and not SIMESH_OPENMP_ENABLED:
        raise RuntimeError("parallel preparation requires an OpenMP build")
    start = time.perf_counter()
    owner = AMRMesh(3, block, domain, lower, upper, 2, nfields, forest)
    allocated = time.perf_counter()
    if (interiors.shape[0] != owner.nleafs or interiors.shape[1] != nfields or
            interiors.shape[2] != block[0] or interiors.shape[3] != block[1] or
            interiors.shape[4] != block[2]):
        raise ValueError("interiors must match the private analysis mesh")
    # Construction has already cleared this private buffer, including padding.
    np.asarray(owner.data)[:, 2:2+block[0], 2:2+block[1], 2:2+block[2], :] = (
        np.transpose(np.asarray(interiors), (0, 2, 3, 4, 1)))
    loaded = time.perf_counter()
    # This private factory admits only 3D, two-halo meshes.
    owner.getbc(workers, executor)
    exchanged = time.perf_counter()
    if owner._datac_ptr is not NULL:
        # This owner has not escaped; no external coarse view can exist.
        owner.datac = None
        free(owner._datac_ptr)
        owner._datac_ptr = NULL
    released = time.perf_counter()
    return owner, (allocated-start, loaded-allocated, exchanged-loaded, released-exchanged)
