...
from .slices import Plane, iter_uniform, _uniform_geometry
from .tracing import iter_traces, Termination, _validate_vector, _validate_inputs
...
def sample(fields, points, *, workers=1, memory_limit=None):
    ...
    _points(points)
    values,owners,valid = sample_values(fields,points.positions,workers=workers,
                                       memory_limit=remaining(memory_limit,points.nbytes))
    return SampledPoints(points,values,owners,valid,fields.fields,fields.value_identity)
...
def field_map(fields, surface, *, workers=1, memory_limit=None):
    """Sample a field or precomputed diagnostic on a Plane or PointSet."""
    return sample(fields,_surface(fields,surface,memory_limit),workers=workers,memory_limit=memory_limit)
...
def surface_diagnostics(fields, surface, *, quantities=("q","twist"), memory_limit=None, **controls):
    ...
    return connectivity(fields,_surface(fields,surface,memory_limit),quantities=quantities,
...
def bottom_diagnostics(fields, shape=(128,128), *, quantities=("q","twist"), ids=None,
    ...
    return connectivity(fields,points,quantities=quantities,memory_limit=memory_limit,**controls)
...
def uniform_grid(fields, resolution, *, bounds=None, workers=1, tile_rows=64, memory_limit=None):
    """Collect a sampled uniform volume; iter_uniform remains the streaming API."""
    require_fields(fields,halo=1)
    resolution,lower,upper = _uniform_geometry(fields.mesh,resolution,bounds)
    output_bytes = math.prod(resolution)*(8*len(fields.fields)+1)+48
    admit(fields.nbytes+fields.mesh.nbytes+output_bytes,memory_limit,"uniform volume")
    slabs = iter_uniform(fields,resolution,bounds=(lower,upper),workers=workers,tile_rows=tile_rows,
                         memory_limit=remaining(memory_limit,output_bytes))
    first = next(slabs)
    try:
        values = np.empty((*resolution,len(fields.fields)))
        valid = np.empty(resolution,dtype=bool)
        index,slab = first
        values[:,:,index] = slab.values
        valid[:,:,index] = slab.valid
        del first,slab
        ...
        slabs.close()
    return UniformResult(values,valid,lower.copy(),upper.copy(),fields.fields,fields.value_identity)
...
def trace(fields, points, *, direction="both", step, max_steps=1000, max_length=np.inf,
    ...
    pieces = [None]*(2*len(points))
    retained = 0
    for side in range(2):
        rows = np.flatnonzero(requested[:,side])
        for start in range(0,len(rows),seed_batch):
            selected = rows[start:start+seed_batch]
            batches = iter_traces(fields,np.ascontiguousarray(positions[selected]),seed_ids=points.ids[selected],
                step=step,max_steps=max_steps,trajectories=True,direction=2*side-1,seed_batch=len(selected),
            ...
            raw_bytes = array_bytes(vars(result).values())
            new_bytes = int(point_counts.sum())*24
            admit(base+retained+raw_bytes+new_bytes,memory_limit,"packed trace output")
            for index,row in enumerate(selected):
                path = result.trajectories[index,:point_counts[index]].copy()
                if len(path):
                    path[0] = points.positions[row]
                pieces[2*int(row)+side] = path
                status[row,side] = result.termination[index]
            retained += new_bytes
            del result
    admit(base+3*retained,memory_limit,"collected trace output")
    counts = [0 if path is None else len(path) for path in pieces]
    ...
    for index,path in enumerate(pieces):
        if path is not None:
            packed[offsets[index]:offsets[index+1]] = path
