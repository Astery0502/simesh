...
top-level/application exports and result-file registration are separate work.
...
arithmetic. The native sampler evaluates all columns of the supplied group in
each batch, even when retaining only selected columns; this scratch is budgeted.
...
branch/joined displays have their own optional admission check. There is no
automatic plotting, result-file registration, streaming result sink, field
composition, or modification of the tracing algorithm in this module.
