...
def test_recovery_reductions_los_and_persistence(tmp_path):
    ...
    density,temperature = 1e-15,8e5
    nh = density/((1+4*composition.helium_abundance)*PROTON_MASS_G)
    pressure = (2+3*composition.helium_abundance)*nh*BOLTZMANN_ERG_K*temperature
    state = lambda x,y,z: (density,(0.,0.,2e6),pressure,(0.,0.,1.))
    ...
    length = sm.LengthUnits(units.length_cm,"cm")
    ...
    np.testing.assert_allclose(mass.value,2e12)
    ...
    thermal = sm.thermal_fields(state_fields,state_fields,density_component=0,temperature_component=1,
        density_unit_g_cm3=1.,temperature_label="recovered ideal MHD temperature")
    ...
    current = sm.current_density(magnetic, units=units.magnetic_si)
    scaled = sm.derive_many(state_fields, {"temperature_MK": "MK", "density_cgs": "g cm^-3"},
        lambda ctx: {"density_cgs": ctx.field("density"),
    ...
    np.testing.assert_allclose(profiles.values, np.broadcast_to([density, temperature*1e-6, 0.],
    ...
    for seed_id in profiles.seed_ids:
        for direction in (-1, 1):
            ...
            np.testing.assert_allclose(branch.arclength, expected*units.length_cm)
