...
def test_nonlinear_amr_map_is_stable_under_step_and_stencil_refinement():
    ...
    coarse = sm.qsl(ready, seeds, bounds=BOX, delta=1e-4, step_fraction=.125)
    fine = sm.qsl(ready, seeds, bounds=BOX, delta=5e-5, step_fraction=.0625)
...
def test_requested_surface_at_partial_coverage_edge_and_input_contracts():
    ...
    interior_only_twist_disabled = sm.qsl(replace(partial, valid_halo=1), seeds,
        bounds=([-1,-1,0],[1,1,upper_z]), twist=False)
...
def test_valid_stencils_keep_their_rows_among_failed_seeds():
    ...
    result = sm.qsl(ready, seeds, twist=False, workers=2, seed_batch=3)
