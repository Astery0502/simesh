...
**当前状态：已关闭，两个候选均不采用。** 用户在看到收益幅度后明确选择保留
现有 N4 实现，优先减少维护负担。四工作者完整流程中位数改善约为 B/curl
3.2%、热 LOS 4.9%；这是有限收益，不称为完全没有收益。主目录未合入候选。
独立源码评审和各候选前后比较已完成；整合构建通过 47 项、跳过 2 项，但
组合性能实验在启动前取消。两个探索 worktree 和临时整合 worktree 退出，
源码提交、必要证据和原始测量已归档。详见[收口记录](../evidence/sampling-reuse-review.md)。
下文只保留原启动约定，不再授权继续实施或等待整合。
