...
| R9 | [L results](pipeline-results.md#l-full-domain-los-integration-of-a-local-response), U07 | External response/traversal references; no completed L path | L1/S5; view/depth/AMR coverage and accumulation |
...
| <a id="l1"></a>L1 | Response/EOS/units/invalid or out-of-range behavior; view orientations, per-pixel depth and ray versus area average; traversal/weights, reconstruction, accumulation precision/order and empty rays | Mixed-level constant-response rays with different views/depths, then independent physical response verification | R9; correct global LOS image |
