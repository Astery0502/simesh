...
class ThermalTests(unittest.TestCase):
    ...
    def test_native_reference_parallel_and_failure_conformance(self):
        ...
        from simesh.utils.lib.analysis.thermal_rays import openmp_build_info
        ...
        for direction in ([1.,0.,0.],[1.,.3,.2],[-1.,-.8,.6],[0.,0.,-1.]):
            ...
            backends = ('auto','threadpool','openmp') if openmp_build_info()['enabled'] else ('auto','threadpool')
            for backend in backends:
                for schedule in ('static','dynamic'):
                    for workers in (2,4):
                        parallel = integrate_thermal_los(state,plane,direction,length_unit_cm=1e8,
                            near=near,workers=workers,backend=backend,schedule=schedule)
                        for name in ('values','samples','status','entry','exit'):
                            np.testing.assert_array_equal(getattr(parallel,name),getattr(native,name))
            if not openmp_build_info()['enabled']:
                with self.assertRaises(RuntimeError):
                    integrate_thermal_los(state,plane,direction,length_unit_cm=1e8,backend='openmp')
