...
def main():
    ...
    parser.add_argument('--output',type=Path,default=results/'install.json')
...
from simesh.analysis import (open_source, PreparedPool, trace, open_prepared, sample, global_curl,
...
with tempfile.TemporaryDirectory() as directory:
    ...
    with open_source(path,field_names=["b1","b2","b3"],value_cache_capacity=8) as source:
        ...
        np.testing.assert_array_equal(global_curl(source,batch_size=3).values,0.)
    fields=open_prepared(path,field_names=["b1","b2","b3"])
    np.testing.assert_array_equal(sample(fields,np.array([[.2,.4,.5]]))[0],[[1.,0.,0.]])
    regional=open_prepared(path,field_names=["b1","b2","b3"],
                           bounds=([0.,0.,0.],[.49,.49,.49]))
    assert len(regional.leaf_ids)==1
    np.testing.assert_array_equal(sample(regional,np.array([[.2,.4,.4]]))[0],[[1.,0.,0.]])
