...
Experimental full-domain analysis uses `src/simesh/amrvac/analysis_read.py` to
read contiguous batches of complete v5 records and extract selected ordinary
interiors. It validates record shape/length and preserves endian-normalized
float64 bits. Its eager result feeds canonical bulk ghost preparation; regional
and bounded analysis retain their selective reader. This does not change the
canonical public read/write entrypoints described above.
