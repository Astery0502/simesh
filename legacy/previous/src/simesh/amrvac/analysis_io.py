...
def open_source(path,*,field_names=None,field_indices=None,field_units=None,
    ...
    """
    with _open_source(path,field_names=field_names,field_indices=field_indices,
                      field_units=field_units,support_capacity=support_capacity,
                      value_cache_capacity=value_cache_capacity,budget_bytes=budget_bytes) as opened:
        yield opened[0]


@contextmanager
def _open_source(path,*,field_names=None,field_indices=None,field_units=None,
                 support_capacity=128,value_cache_capacity=0,budget_bytes=2*1024**3):
    """Share the checked file reader with the owned full-domain loading path."""
...
def open_prepared(path,*,field_names=None,field_indices=None,field_units=None,
                  bounds=None,budget_bytes=2*1024**3):
    """Read selected fields and prepare them once for repeated native analysis.

    Without bounds, use canonical full-domain bulk preparation. A physical box
    selects complete intersecting leaves using the exact-phase regional provider,
    which reads their original AMR support without imposing a cut boundary.
    Both paths return detached readonly fields and close the input file.
    """
    from .analysis import prepare_resident
    start = time.perf_counter()
    with _open_source(path,field_names=field_names,field_indices=field_indices,field_units=field_units,
                      budget_bytes=budget_bytes) as (source,index,reader):
        opened = time.perf_counter()
        mesh = source.mesh
        count = mesh.leaf_count
        k = len(source.fields)
        if bounds is not None:
            from simesh.analysis.fields import prepare_region
            product = prepare_region(source,bounds,np.arange(k,dtype=np.int64),
                                     budget_bytes=budget_bytes)
            stats = {**product.preparation_stats,"open_seconds":opened-start,
                     "file_to_prepared_seconds":time.perf_counter()-start,
        ...
        from .analysis_read import READ_BATCH_BYTES, read_full_interiors
        largest_record = int(np.max(np.diff(np.r_[index.block_offsets,index.file_identity[2]])))
        read_scratch = max(source.scratch_bytes(np.arange(k,dtype=np.int64),0),
                           2*max(READ_BATCH_BYTES,largest_record))
        ...
        raw, io_stats = read_full_interiors(reader,np.asarray(source.original_field_ids,dtype=np.int64))
