...
def prepare_region(source, bounds, field_ids, *, budget_bytes=2*1024**3):
    """Prepare every native leaf intersecting a half-open physical box once.

    Coverage expands to complete intersecting leaves, with two valid halo layers.
    Support is read from the original mesh, including outside the selected box;
    the selection never creates a physical boundary. The detached result can be
    reused without a source or cache. Samples outside its leaf coverage are
    invalid, and tracing reports missing coverage rather than loading more data.
    """
    bounds = np.asarray(bounds, dtype=float)
    if (bounds.shape != (2, 3) or not np.isfinite(bounds).all() or
            np.any(bounds[0] >= bounds[1])):
        raise ValueError("bounds must contain finite, strictly ordered 3D lower/upper vectors")
    mesh = source.mesh
    selected = np.flatnonzero(np.all(
        (mesh.bounds[:, 1] > bounds[0]) & (mesh.bounds[:, 0] < bounds[1]), axis=1))
    if not len(selected):
        raise ValueError("bounds do not intersect the source domain")
    return prepare(source, selected, field_ids, budget_bytes=budget_bytes)
