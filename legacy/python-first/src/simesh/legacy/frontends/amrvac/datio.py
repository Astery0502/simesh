...
def write_header(fi, header):
    ...
    if header["datfile_version"] >= 5:
        fmt = ALIGN + header["ndim"] * "i"
        periodic = np.asarray(header["periodic"], dtype=int)
        ...
        fmt = ALIGN + "i"
        packed_data = struct.pack(fmt, int(header["staggered"]))
        fi.write(packed_data)
...
def write_forest_tree(fi, header, forest, tree):
    ...
    block_lvls = np.asarray(block_lvls, dtype=int)
    block_ixs = np.asarray(block_ixs, dtype=int)
    block_offsets = np.asarray(block_offsets, dtype=int)
