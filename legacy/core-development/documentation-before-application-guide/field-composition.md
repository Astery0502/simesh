...
These functions are available from `simesh` and through focused imports:
...
allocations, output, leaf directory and temporary publication-validation indices.
A small view into a large input backing
...
For sampling-only selection, pass `components` to the sampler, plane or uniform-grid
consumer to avoid a global field copy. Restrict Sources before raw cache allocation
with `select_source`; see [selected delivery](selected-delivery.md).
