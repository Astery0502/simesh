...
The [capability and output guide](capabilities-and-outputs.md) lists exact
example filenames, readers and the context to deliver with numerical results.
...
Save the complete `LineProfiles` with `save_result` and restore it with
`load_result`; nested curves, selected component definitions, distances and
validity masks are retained. Individual branch/joined views are not standalone
result-file types. See [profile persistence](line-profiles.md#save-restore-or-deliver-by-shard).
...
The example writes `thermal.result.npz` (a `RayResult`, with quantitative
summaries and the MHD model in caller metadata) and `streamlines.result.npz`
(a velocity `LineSet`). Load both with `load_result`. Profiles are calculated
and checked in memory; the example does not write them. To retain its computed
`profiles`, add `sm.save_result(output / "profiles.result.npz", profiles)`.
It prints the analytic checks and does not create a standalone summary file.
