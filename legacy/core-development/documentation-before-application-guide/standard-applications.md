...
grids and LOS. Preparation schemes remain explicit; native preparation currently
uses continuous physical boundaries. See the
[capability and output guide](capabilities-and-outputs.md) for the full inventory
and file producer/reader pairs.
...
`products.npz` and `summary.json`, including physical scales, geometry and status.
`products.npz` is an example-specific array bundle: read it with
`numpy.load(..., allow_pickle=False)`, not `load_result`. The summary is not an
automatic capture of every calculation parameter. To preserve application types
and nested geometry, use [versioned result files](result-files.md) instead.
...
The exact output files and a readback example are listed in the
[output guide](capabilities-and-outputs.md#representative-examples-and-their-exact-files).
