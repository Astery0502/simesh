...
This is an independent, experimental Python toolkit for AMR scientific analysis
and numerical result delivery. Start with the
[physical capability inventory and output guide](../capabilities-and-outputs.md)
for supported calculations, file formats, readers and complete examples.

Its native workflows provide immutable file/array sources, exact-phase regional and
...
interfaces; use the [application guide](../development/application-development.md).
...
See [application interfaces](../applications.md). QSL/twist calculations do
...
[standard applications and the complete example](../standard-applications.md).

Quantitative workflows now include [native AMR integrals/statistics](../reductions.md),
[explicit ideal-MHD recovery](../mhd-thermodynamics.md), and
[versioned application result files](../result-files.md). These APIs are
available through `simesh` as well as their focused modules. The
[combined workflow](../quantitative-workflow.md) recovers a state, reduces its
interiors, generates thermal LOS and streamlines, and reloads saved products.

[Field composition](../field-composition.md) adds owned component selection,
leaf-aligned merging and multi-output pointwise recipes. Use
[line profiles](../line-profiles.md) to sample these quantities along stored
...
The repository root now builds and installs this package as `simesh`. The
preceding main package and its earlier Python implementation are archived
separately under [legacy/](../../../README.md). Runtime code and build inputs live
under `src/`; no archived package or historical `simesh_rewrite` provider is
needed after installation. See [architecture](../../../../docs/architecture.md) for the
current Python/Cython layout and [migration](../../../../MIGRATION.md) for moved imports.
...
For explicit interior/halo requirements, source field selection, caller outputs
and per-seed result shards, see [selected delivery](../selected-delivery.md).

`Source.metadata` exposes detached, read-only snapshot time, model parameters,
the original field directory and file-header information. It survives source
closure and can be passed to `write_amrvac` or recorded with analysis results.
See [snapshot metadata](../snapshot-metadata.md).
...
This path reads contiguous record batches directly into the interior region of
an independently owned final NumPy array. Read buffers are released before
allocating coarse exchange scratch. The published result retains neither the exchange bindings, their
...
with local-sphere maps. See [magnetic connectivity](../connectivity.md) for
...
[fixed core design](../development/next-generation-design.md) and
[application guide](../development/application-development.md) are the active
development references. Earlier specifications, experiments and N1–N4 acceptance
records are preserved in the [archive](../development/archive/README.md).
