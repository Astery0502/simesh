# Migrating to the root package

For the current physical feature inventory and the writer/reader pair for each
output product, see the [capability and output guide](../capabilities-and-outputs.md).
This page describes interface migration and compatibility boundaries.

The repository root now installs the implementation previously developed under
`analysis-core/`. Both generations use the distribution and import name
`simesh`; use the root environment for current work and a separate environment
for any historical comparison. The archived sources are described in
[legacy/README.md](../../../README.md). Recreate environments and rebuild Cython
extensions after the move; old editable installs and virtualenv script paths
are not a portable migration mechanism.

## Directory and import changes

| Previous location or import | Current location or import |
| --- | --- |
| `analysis-core/src`, `tests`, `examples`, build files | Root `src`, `tests`, `examples` and build files |
| `analysis-core/docs/` | Root `docs/` user guides |
| `docs/analysis-core/` | `docs/development/` collaboration and historical evidence |
| Previous root implementation and `rewrite/` | `legacy/previous/` |
| Previous `src/simesh/legacy/` | `legacy/python-first/src/simesh/legacy/` archival source |
| `simesh.utils.configurations` | `simesh.tools.configurations` |
| `simesh.utils.openmp_enabled`, `openmp_build_info` | `simesh.amrvac.openmp_enabled`, `openmp_build_info` for the retained Dataset extension |
| `simesh.utils.lib.amr` and support headers | Private `simesh.amrvac._mesh`, with headers in the same directory |

The old `simesh.utils` namespace has been removed without forwarding aliases.
Use public Dataset operations rather than importing private `_mesh` internals.
Native `simesh._kernels` and the public scientific/result APIs keep their names
and numerical behavior. The existing eight AMRVAC file/Dataset entrypoints also
retain their signatures. No runtime import resolves into the repository archive.
...
interfaces. See [application interfaces](../applications.md) for selection,
...
| Existing analytic array configurations | `simesh.tools.configurations` | Retained NumPy helpers; no native AMR preparation dependency |

The compatibility implementation is bundled under `simesh.amrvac` and its
`simesh.amrvac._mesh` dependencies. These modules are loaded when requested;
...
| --- | --- |
| `open_source(path, field_names=...)` | `open_amrvac(path, fields=...)` or borrowed `select_source(source, fields)` before caching/preparation |
...
order, and excludes halo values. Its required `metadata` accepts the detached
`SnapshotMetadata` available as `source.metadata`, or an existing v5 header dict
...
Native file sources expose snapshot time, model parameters and the original
header without creating a Dataset. Source adapters share this read-only metadata;
Dataset snapshots copy it, and array sources leave it unspecified by default.
Keep metadata separately from Fields, and use `metadata.to_dict()` to record it
in application result files. See [snapshot metadata](../snapshot-metadata.md).
...
- Compatibility AMR extensions are built serially, even with
  ...
  OpenMP option. `simesh.amrvac.openmp_build_info()` reports compatibility status;
...
see [magnetic connectivity](../connectivity.md) for methods and validity.
